...
class TeacherLearningHubAnalyticsService
{
    ...
    public function dashboard(array $teacher, ?string $classId = null, int $days = 30): array
    {
        ...
        $defaults = [
            ...
            'students' => ['id' => '', 'name' => 'Student', 'grade_level' => null, 'classes' => '', 'last_practiced_at' => null],
    ...
    public function studentDashboard(
        ...
        $days = in_array($days, [7, 30, 90, 180], true) ? $days : 30;
        try {
            $result = $this->supabase->adminRpcResult('teacher_learning_hub_student_analytics', [
                'p_teacher_id' => (string) ($teacher['id'] ?? ''),
            ...
            Log::warning('Student Learning Hub insights could not reach the data service.', [
                'teacher_id' => $teacher['id'] ?? null,
                'student_id' => $studentId,
                'exception' => $exception::class,
            ]);

            return $this->emptyStudentState(
                $studentId,
                $days,
                $classId,
                false,
                'This student’s Learning Hub statistics are temporarily unavailable. Try again shortly.'
            );
        }

        if ($result['error'] !== null || !is_array($result['data'][0] ?? null)) {
            Log::warning('Student Learning Hub insights returned an unavailable or invalid result.', [
                'teacher_id' => $teacher['id'] ?? null,
                'student_id' => $studentId,
                'status' => $result['status'] ?? null,
            ]);
        ...
        $payload = $result['data'][0];
        $topics = $this->topicMap();
        foreach (['topics', 'recent_activity'] as $collection) {
            $payload[$collection] = $this->rows($payload[$collection] ?? null);
            foreach ($payload[$collection] as &$row) {
                $key = (string) ($row['competency_key'] ?? '');
                $row['topic_title'] = $topics[$key]['title'] ?? $this->humanizeKey($key);
        ...
        $state = $this->emptyStudentState($studentId, $days, $classId, true);
        if (is_array($payload['student'] ?? null)) {
            $state['student'] = array_replace($state['student'], $payload['student']);
        }
        if (is_array($payload['summary'] ?? null)) {
            $state['summary'] = array_replace($state['summary'], $payload['summary']);
        }
        foreach (['classes', 'topics', 'daily_activity', 'recent_activity'] as $key) {
            if (is_array($payload[$key] ?? null)) {
                $state[$key] = $this->rows($payload[$key]);
            }
        }

        $defaults = [
            'classes' => ['id' => '', 'name' => 'Class', 'grade_level' => null],
            'topics' => [
                'competency_key' => '',
                'topic_title' => 'Topic',
                'topic_icon' => 'fa-book-open',
                'mastery' => 0,
                'period_answers' => 0,
                'period_accuracy' => 0,
                'period_hints' => 0,
            ...
            'daily_activity' => ['activity_date' => null, 'answers' => 0, 'accuracy' => 0],
            'recent_activity' => [
                'competency_key' => '',
                'topic_title' => 'Topic',
                'topic_icon' => 'fa-book-open',
        ...
        foreach ($defaults as $key => $rowDefaults) {
            $state[$key] = array_map(
                fn (array $row): array => array_replace($rowDefaults, $row),
                $state[$key]
            );
        }
        $state['timezone'] = (string) ($payload['timezone'] ?? 'UTC');
        $state['generated_at'] = isset($payload['generated_at'])
    ...
    private function emptyStudentState(
        string $studentId,
        int $days,
        ?string $classId,
        bool $configured,
